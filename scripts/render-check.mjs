...
for(const page of ['index.html','eventos.html'])for(const lang of ['es','pt','en']){
 ...
 if(isEvent){el('[data-corporate]').onclick();assert.equal(el('#event-form [name=type]').value,'3');assert.equal(el('#event-form [name=need]').value,'4');assert(decodeURIComponent(clicked).includes('Signature Cocktails'));assert(html.includes('Corporate Events'));assert(html.includes('Gin'));}else{
 el('audit-cta').onclick();assert.equal(el('#lead-form select').value,'5');el('group-cta').onclick();assert.equal(el('#lead-form select').value,'6');assert(html.includes('+100%'));assert(html.includes('2021')&&html.includes('2024'));assert(!html.includes('3×'));
